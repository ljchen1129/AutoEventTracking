...
class LinkedNodeMap: NSObject {
    ...
    func insertNodeAtHead(_ node: LinkedNode) {
        guard let key = node.key, let cost = node.cost else {
            return
        }
    ...
    /// 删除尾节点
    func removeTailNode() {
        guard let tempTail = tail,
              let key = tempTail.key,
              let cost = tempTail.cost else { return }
...
extension MemoryCache {
    ...
    public func setObject(_ object: Any, forKey key: AnyHashable, withCost cost: UInt) {
        ...
        linedMap.totalCost -= node.cost ?? 0
...
extension MemoryCache {
    ...
    public func trim(withAge age: TimeInterval) {
        guard let wrapTailTime = linedMap.tail?.time else {
            return
        }
        ...
        if (ageLimit <= 0) {
            linedMap.removeAll()
            finish = true
        } else if (linedMap.tail == nil || (now - wrapTailTime) <= age) {
            finish = true
        }
        pthread_mutex_unlock(&lock)
        if (finish) { return }
        ...
        while (finish == false) {
            if (pthread_mutex_trylock(&lock) == 0) {
                if ((linedMap.tail != nil) && (now - wrapTailTime) > age) {
                    linedMap.removeTailNode()
                } else {
                    finish = true
                }
