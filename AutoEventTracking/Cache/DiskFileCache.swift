...
class DiskFileCache: NSObject, Cacheable {
    ...
    var totalCost: UInt {
        get {
            ...
            var fileSize: UInt = 0
            do {
                guard let cacheDir = fileCacheDir else {
                    return 0
                }
...
extension DiskFileCache {
    func contain(forKey key: AnyHashable) -> Bool {
        guard let wrapKey = key as? String, let fileUrl = creatFileUrl(wrapKey) else {
            return false
        }
    ...
    func object(forKey key: AnyHashable) -> Data? {
        guard let wrapKey = key as? String,
              let fileUrl = creatFileUrl(wrapKey) else { return nil }
    ...
    func setObject(_ object: Data, forKey key: AnyHashable, withCost cost: UInt) {
        guard let wrapKey = key as? String,
              let fileUrl = creatFileUrl(wrapKey) else { return }
    ...
    func removeAllObject() {
        guard let dir = fileCacheDir,
              let fileArray = FileManager.default.subpaths(atPath: dir.path) else { return }
